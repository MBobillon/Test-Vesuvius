...
static
double st_signedVolumeTetrahedon(  const Point& a,  const Point& b,   const Point& c)
{
   return (a.cross(b)).dot(c);
}
double st_computeVolumeMesh(const std::vector<Point>& triangles)
{
    double volume = 0.0;
    for (size_t i = 0; i < triangles.size(); i += 3)
       volume += st_signedVolumeTetrahedon (triangles[i],triangles[i + 1],triangles[i + 2]);  
